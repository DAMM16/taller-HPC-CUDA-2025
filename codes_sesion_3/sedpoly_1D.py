...
def flujovert(j, phil, phir, num_capas, num_especies, d, d_f, diam, g):
    """
    En tu Fortran num_capas se pasa pero no se usa realmente en la fórmula final.
    """
    # V(i)=vMLB(i,phir,...)
...
def solver(phil, phir, tF, num_especies, g, d, diam, df, num_capas=0):
    ...
    if tF == 0:
        ...
        for i in range(num_especies):
            h[i] = flujovert(i, phil, phir, num_capas, num_especies, d, df, diam, g)
...
def Flujo(phi, num_celdas, num_especies, diam, d, g, df, tF=2):
    ...
    # Borde izquierdo: en tu Fortran pones F(:,1)=0
    # pero igual llamas solver con phil=phi(:,1), phir=phi(:,1) y después lo ignoras.
    F[:, 0] = 0.0
    ...
    # Borde derecho: en tu Fortran phil=0, phir=0 y F(:,nc+2)=h (cero)
...
path = "/home/victor/Cosas_Importantes/Charlas/Taller_HPC"
...
save_lines_csv(path + "/examples/lines_0.csv", x, phi)
...
while tiempo < tf:
    ...
    if abs(tiempo - ts) < 1e-12 or abs(tiempo - tf) < 1e-12:
        ...
        save_lines_csv(f"{path}/examples/lines_{n_iter_save}.csv", x, phi)
        n_iter_save = n_iter + 1
